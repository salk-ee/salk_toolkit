...
# %% auto 0
__all__ = ['estimate_legend_columns_horiz_naive', 'estimate_legend_columns_horiz', 'boxplot_vals', 'boxplot_manual',
           'maxdiff_manual', 'columns', 'stacked_columns', 'diff_columns', 'massplot', 'make_start_end', 'likert_bars',
           'kde_bw', 'kde_1d', 'density', 'violin', 'cluster_based_reorder', 'matrix', 'corr_matrix', 'lines',
           'draws_to_hdis', 'lines_hdi', 'area_smooth', 'likert_aggregate', 'likert_rad_pol', 'barbell', 'geoplot',
           'geobest', 'fd_mangle', 'facet_dist', 'ordered_population', 'marimekko']
...
def cat_to_cont_axis(data, fx):
    ...
    if x_cont.notna().all():
        ...
        x_axis = alt.X(field=fx["col"], type='quantitative', title=None, axis=alt.Axis(labelAngle=0, values=list(data[fx["col"]].unique())))
    else:
...
def lines(data, value_col='value', facets=[], smooth=False, width=800, tooltip=[], val_format='.2f',):
    ...
    plot = alt.Chart(data).mark_line(point=points, interpolate=smoothing).encode(
        x=x_axis,
        y=alt.Y(field=value_col, type='quantitative', title=(value_col if len(value_col)<20 else None),
                        axis=alt.Axis(format=val_format)),
        tooltip=tooltip,
        **({'color':alt.Color(field=fy["col"], type='nominal', scale=fy["colors"], sort=fy["order"],
                        legend=alt.Legend(orient='top',
                                    columns=estimate_legend_columns_horiz(fy["order"],width)))} if len(facets)==2 else {})
    )
    return plot
...
def lines_hdi(data, value_col='value', facets=[], width=800, tooltip=[], val_format='.2f', hdi1=0.94, hdi2=0.5):
    ...
    plot = alt.Chart(hdf).mark_area(interpolate='basis').encode(
        x=x_axis,
        y=alt.Y('lo:Q',
            axis=alt.Axis(format=val_format, title=(value_col if len(value_col)<20 else None),),
            title=value_col
            ),
        y2=alt.Y2('hi:Q'),
        tooltip=[
...
def area_smooth(data, value_col='value', facets=[], width=800, tooltip=[]):
    ...
    plot=alt.Chart(data
            ...
            order=alt.Order('order:O'),
            color=alt.Color(field=fy["col"], type='nominal',
                legend=alt.Legend(orient='top',columns=estimate_legend_columns_horiz(fy["order"],width)),
                sort=fy["order"], scale=fy["colors"]
                ),
            tooltip=tooltip
        )
    return plot
...
def linevals(vals, value_col, n_points, dim, cats, ccodes=None, ocols=None, boost_signal=False, gc=False, weights=None):
...
# %% ../nbs/03_plots.ipynb 65
@stk_plot('ordered_population', data_format='raw', factor_columns=3, aspect_ratio=(1.0/1.0), plot_args={'group_categories':'bool', 'full_data':'bool'}, n_facets=(0,1), no_question_facet=True)
def ordered_population(data, value_col='value', facets=[], tooltip=[], outer_factors=[], group_categories=False, full_data=False):
    f0 = facets[0] if len(facets)>0 else None
    ...
    if not full_data and len(data)>maxn: data = data.sample(maxn,replace=False,random_state=42)
