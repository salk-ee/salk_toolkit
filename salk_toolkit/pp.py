...
# %% ../nbs/02_pp.ipynb 9
special_columns = ['id', 'weight', 'draw', 'training_subsample', 'original_inds', '__index_level_0__', 'group_size', 'diff']
        ...
        if is_categorical:
            ...
            if agg_fn in ['mean','sum']: # Use weighted sum to compute both sum and mean
                ...
                data = (
                            ...
                            .with_columns(pl.col(res_col)/pl.col(weight_col).alias(res_col))
                            .with_columns((pl.col(res_col) + pl.col("reverse_"+res_col)).alias("diff"))
                        )
...
def create_plot(pparams, pp_desc, alt_properties={}, alt_wrapper=None, dry_run=False, width=200, height=None, return_matrix_of_plots=False, translate=None):
    ...
    elif 'diff' in data.columns and pp_desc.get('plot') == 'maxdiff':
        if pp_desc.get('factor_cols'):
            q = pp_desc['factor_cols'][0]
            order = data.groupby(q,observed=True)['diff'].mean().sort_values(ascending=False).index
