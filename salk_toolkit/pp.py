...
def _plot_fn_accepts_plot_input(plot_fn: Callable[..., Any]) -> bool:
    """Detect whether a plot implementation expects ``PlotInput`` as its first argument."""

    try:
        params = list(inspect.signature(plot_fn).parameters.values())
    ...
    if annotation is PlotInput:
        return True
    if getattr(annotation, "__name__", "") == "PlotInput":
        return True
    return first.name in {"p", "plot_params", "pparams", "pi"}
...
def _ensure_plot_args_sync(func: Callable[..., Any], decorator_kwargs: Dict[str, Any]) -> None:
    ...
    if not expected:
        return
    if not _plot_fn_accepts_plot_input(func):
        return
...
def get_plot_meta(plot_name: str) -> PlotMeta:
...
def _update_data_meta_with_pp_desc(
    ...
    if not isinstance(data_meta, DataMeta):
        if isinstance(data_meta, dict):
            if "file" not in data_meta and "files" not in data_meta:
                data_meta = dict(data_meta)  # Make a copy
                data_meta["file"] = "__virtual__"
...
def _wrangle_data(
    ...
    """Aggregate filtered data into a structured ``PlotInput`` model."""
...
def create_plot(
    ...
    pi.outer_colors = col_meta.get(factor_cols[0], GroupOrColumnMeta()).colors or {} if factor_cols else {}
    ...
    accepts_plotparams = _plot_fn_accepts_plot_input(plot_fn)
    plot_arg_payload = clean_kwargs(plot_fn, extra_plot_args)
    legacy_base_kwargs: Dict[str, Any] | None = None
    if not accepts_plotparams:
        legacy_base_kwargs = pi.model_dump(mode="python", exclude={"plot_args"})
        legacy_base_kwargs.update(extra_plot_args)

    def _call_plot_fn(
        data_override: pd.DataFrame | None = None,
    ) -> AltairChart | List[List[AltairChart]] | PlotInput:
        if accepts_plotparams:
        ...
        assert legacy_base_kwargs is not None
        payload = dict(legacy_base_kwargs)
        if data_override is not None:
            payload["data"] = data_override
        payload = clean_kwargs(plot_fn, payload)
        return plot_fn(**payload)
