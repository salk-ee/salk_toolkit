...
def read_annotated_data(
    ...
    # No-op for json/yaml (already row_id-indexed); mints ids for parquet predating stable ids
...
def _perform_merges(
    ...
    for merge_idx, ms in enumerate(merges):
        ...
        # Drop provenance columns from the merge side to avoid suffix collisions; keep the left df's
        ndf = ndf.drop(
...
def read_and_process_data(
    ...
    if isinstance(desc_obj, DataDescription) and desc_obj.data is not None:
        ...
        # Categories are already reconciled - _load_data_files ran _fix_meta_categories on these frames
    ...
    if isinstance(desc_obj, DataDescription):
        # One shared namespace on purpose (unlike HookEnv): preprocessing names stay visible downstream
