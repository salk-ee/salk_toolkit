...
def _gather_source(
    ...
    for file_code, file_raw_data in bundle.frames.items():
        # A dict maps file codes ('default' then orig_cn as fallbacks); a string applies to all files
...
def _apply_exclusions(ndf_df: pd.DataFrame, meta_obj: DataMeta, opts: ProcessOpts) -> pd.DataFrame:
    ...
    # Keyed by row id, so exclusions compose across nesting: an already-filtered id matches nothing
