...
def _read_files_from_description(
    ...
    Args:
        desc: File description object (DataMeta or DataDescription) that specifies
            which files to load via `file` or `files` attributes.
        path: Optional path to resolve relative file paths.
        ignore_exclusions: Whether to ignore exclusion rules.
        only_fix_categories: Whether to only fix categorical types.
        add_original_inds: Whether to add original indices.

    Returns:
        Tuple of (DataFrame, DataMeta or None, extra info dict).
    """
...
def _load_data_files(
    ...
    Returns:
        A tuple of:
        - DataFrame: Concatenated data from all files, with additional columns added for
          file identification (file_ind, file_code) and any extra fields from file descriptions.
        - DataMeta | None: Merged metadata from all files (if any files had metadata),
          or None if no metadata was found. When multiple files have metadata, uses the
          metadata from the last file as base.
        - dict[str, object]: Extra information dictionary containing metadata fields from
          file formats that provide it (e.g., SPSS/Stata file metadata stored in einfo).
    """
...
def _create_new_columns_and_metas(
    ...
    Args:
        ...
        group: ColumnBlockMeta object with create block.
...
def _meta_to_serializable(meta_obj: DataMeta) -> dict[str, object]:
    """Convert DataMeta object to dict with structure and columns in list format for serialization/back-compat.

    Converts both structure and columns from dict format back to list format
    to match the original input format expected by tests.
    Also converts Color objects to strings for backward compatibility.

    Args:
        meta_obj: DataMeta Pydantic object.

    Returns:
        Dictionary with structure and columns as lists (original format).
    """
    # Convert Color objects to strings for backward compatibility using JSON roundtrip
    # This converts Pydantic Color objects to strings while preserving other Python types
    meta_dict = meta_obj.model_dump(mode="json")

    # Convert structure from dict to list format
    if "structure" in meta_dict and isinstance(meta_dict["structure"], dict):
        structure_list = []
        for block_dict in meta_dict["structure"].values():
            ...
            if "columns" in block_dict and isinstance(block_dict["columns"], dict):
                block_dict["columns"] = cs_dict_to_lst(block_dict["columns"])
            structure_list.append(block_dict)
        meta_dict["structure"] = structure_list

    # JSON roundtrip to get Python dict (Colors are already strings from mode="json")
    meta_dict = json.loads(json.dumps(meta_dict))
    return meta_dict
...
def _process_annotated_data(
    ...
    constants: dict[str, object] = dict(getattr(meta_obj, "_constants_cache", {}))
    ...
    for group_name, group in meta_obj.structure.items():
        ...
        # group.columns is Dict[str, Tuple[str, ColumnMeta]]
        for cn, (sn, col_meta) in group.columns.items():
            # Work with Pydantic objects directly
            # Merge scale metadata with column-specific metadata to get final column metadata
            scale_meta = group.scale if group.scale is not None else None
            # Merge scale defaults with column overrides
            mcm = merge_pydantic_models(scale_meta, col_meta)
            ...
            if mcm.categories and not _is_series_of_lists(s):
                # Apply categories to the series
                cats = list(s.unique()) if mcm.categories == "infer" else mcm.categories
                if isinstance(cats, list):
                    # If series is numeric and categories are string numbers, map to nearest category
                    if pd.api.types.is_numeric_dtype(s) and all(
                        isinstance(c, str) and c.replace(".", "").replace("-", "").isdigit() for c in cats
                    ):
                        try:
                            cat_nums = [float(c) for c in cats]

                            # Map each value to nearest category
                            def _map_to_nearest(val: float | int | None) -> str | None:
                                if pd.isna(val):
                                    return None
                                nearest_idx = min(range(len(cat_nums)), key=lambda i: abs(cat_nums[i] - val))
                                return cats[nearest_idx]

                            s = s.apply(_map_to_nearest)
                        except (ValueError, TypeError):
                            pass  # Fall through to regular categorical conversion
                    s = pd.Series(
                        pd.Categorical(s, categories=cats, ordered=mcm.ordered if mcm.ordered is not None else False),
                        name=cn,
                    )
...
def extract_column_meta(data_meta: DataMeta) -> dict[str, GroupOrColumnMeta]:
    """Extract column metadata from a DataMeta object."""
    ...
    for block in data_meta.structure.values():
        ...
        if scale_meta is not None:
            group_payload = scale_meta.model_dump(mode="python")
        group_columns = [f"{col_prefix}{cn}" for cn in block.columns.keys()]
        group_payload["columns"] = group_columns
        res[block.name] = GroupOrColumnMeta.model_validate(group_payload)

        for cn, (_sn, col_meta) in block.columns.items():
            merged_meta = merge_pydantic_models(scale_meta, col_meta)
            col_name = f"{col_prefix}{cn}"
            payload = merged_meta.model_dump(mode="python")
            if scale_meta is not None and col_meta.label is None:
                payload["label"] = None
            res[col_name] = GroupOrColumnMeta.model_validate(payload)
    return res
...
def _get_original_column_names(dmeta: DataMeta) -> dict[str, str]:
    ...
    for block in dmeta.structure.values():
        ...
        for cn, (sn, _col_meta) in block.columns.items():
            col_name = f"{col_prefix}{cn}"
            res[col_name] = sn
    return res
...
def replace_data_meta_in_parquet(
    ...
    if "file" not in nmeta_dict and "files" not in nmeta_dict:
        nmeta_dict = {**nmeta_dict, "file": "__virtual__"}
    ...
    if "original_data" not in meta.extras:
        meta.set_extra("original_data", _meta_to_serializable(ometa))
    meta.data = nmeta

    write_parquet_with_metadata(df, meta, parquet_name)

    return df, meta.payload()
...
def _fix_meta_categories(
    ...
    Args:
        data_meta: Data metadata dictionary or DataMeta object to update.
    ...
    for g in structure.values():
        ...
        updated_columns: dict[str, tuple[str, ColumnMeta]] = {}
        for cn, (sn, col_meta) in columns_raw.items():
            ...
            if full_col_name in df.columns and df[full_col_name].dtype.name == "category":
                ...
                if col_meta.categories == "infer":
                    ...
                    updated_col_meta = col_meta.model_copy(update={"categories": cats})
                all_cats |= set(cats)
            updated_columns[cn] = (sn, updated_col_meta)
...
def _meta_payload_from_obj(meta: dict[str, object] | ParquetMeta) -> dict[str, object]:
    """Return a mutable metadata payload from either dict or ParquetMeta."""
...
def _parse_parquet_meta(raw: dict[str, object]) -> ParquetMeta:
    """Convert raw parquet metadata payload into ParquetMeta."""

    data_payload = raw.get("data")
    if isinstance(data_payload, dict) and "file" not in data_payload and "files" not in data_payload:
        data_payload = {**data_payload, "file": "__virtual__"}
    payload: dict[str, object] = dict(raw)
    if data_payload is not None:
        payload["data"] = data_payload
    return ParquetMeta.from_payload(payload)
...
def write_parquet_with_metadata(df: pd.DataFrame, meta: dict[str, object] | ParquetMeta, file_name: str) -> None:
    ...
    # Ensure DataMeta objects inside meta are serialized
    meta_payload = deepcopy(_meta_payload_from_obj(meta))
    data_meta = meta_payload.get("data")
    if isinstance(data_meta, DataMeta):
        meta_payload["data"] = _meta_to_serializable(data_meta)
...
def read_parquet_metadata(file_name: str) -> ParquetMeta | None:
    """Read metadata bundle from a Parquet file.
    ...
    if custom_meta_key.encode() in schema.metadata:
        ...
        return _parse_parquet_meta(restored_meta)
    return None
...
def read_parquet_with_metadata(
    ...
    if custom_meta_key.encode() in restored_table.schema.metadata:
        ...
        restored_meta = _parse_parquet_meta(restored_meta_payload)
    else:
        restored_meta = None
