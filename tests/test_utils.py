...
class TestDataProcessing:
    ...
    def test_multicol_to_vals_cats(self):
        ...
        df = pd.DataFrame(
            {
                "q1": ["a", "b", "c", None, None, None],
                "q1b": [None, None, None, "c", "b", "a"],
            }
        )
        result = multicol_to_vals_cats(df, col_prefix="q1", reverse_suffixes=["1b"], cat_order=["a", "b", "c"])
    ...
    def test_aggregate_multiselect(self):
        ...
        df2 = pd.DataFrame({"q1_opt1": ["a", "b", "c"], "q1_opt2": ["d", "e", "f"]})
    ...
    def test_deaggregate_multiselect(self):
        ...
        assert bool(df["out_a"].iloc[0]) is True
        assert bool(df["out_b"].iloc[0]) is True
